...
public class FObjToIntMap implements Serializable
{
...
// ObjToIntMap is a copy cat of ObjToIntMap with API adjusted to object keys
    ...
    public static class Iterator {
        ...
        public Object getKey() {
            Object key = keys[cursor];
            if (key == FUniqueTag.NULL_VALUE) { key = null; }
            return key;
        }
    ...
    public FObjToIntMap(int keyCountHint) {
        if (keyCountHint < 0) FKit.codeBug();
        // Table grow when number of stored keys >= 3/4 of max capacity
        int minimalCapacity = keyCountHint * 4 / 3;
        int i;
        for (i = 2; (1 << i) < minimalCapacity; ++i) { }
        power = i;
        if (check && power < 2) FKit.codeBug();
    }
    ...
    public boolean has(Object key) {
        if (key == null) { key = FUniqueTag.NULL_VALUE; }
    ...
    private int findIndex(Object key) {
        if (keys != null) {
            ...
            if (test != null) {
                ...
                for (;;) {
                    if (check) {
                        if (n >= occupiedCount) FKit.codeBug();
                        ++n;
                    }
...
// and enough free space
    private int insertNewKey(Object key, int hash) {
        if (check && occupiedCount != keyCount) FKit.codeBug();
        if (check && keyCount == 1 << power) FKit.codeBug();
        int fraction = hash * A;
        int index = fraction >>> (32 - power);
        int N = 1 << power;
        if (keys[index] != null) {
            ...
            do {
                if (check && keys[index] == DELETED) FKit.codeBug();
                index = (index + step) & mask;
                if (check && firstIndex == index) FKit.codeBug();
    ...
    private void rehashTable() {
        if (keys == null) {
            if (check && keyCount != 0) FKit.codeBug();
            if (check && occupiedCount != 0) FKit.codeBug();
            int N = 1 << power;
            keys = new Object[N];
            values = new int[2 * N];
        }
