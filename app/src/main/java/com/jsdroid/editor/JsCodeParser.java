...
import org.mozilla.javascript.FToken;
import org.mozilla.javascript.FTokenStream;
...
public class JsCodeParser implements Runnable {
    ...
    public void run() {
        ...
        try {
            FTokenStream ts = new FTokenStream(null, codeText.getText()
                    .toString(), 0);
            while (running) {
                if (reparse) {
                    break;
                ...
                try {
                    ...
                    int color = FToken.getColor(token);
