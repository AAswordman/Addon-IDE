...
>>>>>>> aa41e3931568d839484ad45420af53d27f2b4d68
    ...
    private static void addMergeJSON(Ldb ldb, String prefix, ArrayList<Tips> tips) {
        ...
        for (final Tips t:tips) {
            ...
            final byte[] k=(prefix + t.map.get("k")).getBytes();
            final byte[] v=ldb.get(k);
            if (v != null) {
                ...
                if (ox != null) {
                    ...
                    if (str.startsWith("{")) {
                        try {
                            JSONObject jx=new JSONObject(str);
                        ...
                        prepare = addMergeUnknown(str, ox);
                    }

                    if (prepare != null) {
                        ldb.put(k, prepare.toString().getBytes());
                ...
                if (sv != null) {
                    if (sv instanceof JSONObject) {
                        ldb.put(k, new MyJSONObjectTool((JSONObject)sv).toObjAll().toString().getBytes());
                    } else if (sv instanceof JSONArray) {
                        ldb.put(k, new MyJSONArrayTool((JSONArray)sv).toObjAll().toString().getBytes());
                    } else {
                        ldb.put(k, new JSONArray().put(sv).toString().getBytes());
                    }
                }
